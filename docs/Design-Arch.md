...
- 当前实现已落地轻量 `knowledge_relation` 关系表与 repo knowledge map。
- 继续避免重型全量图谱，优先使用轻量图关系或 `LazyGraphRAG` 思路。
- 仅在需要全局关系推理时启用。
...
- 检索和注入全量审计。
- 对敏感知识支持“可索引不可直接展示”。
- 当前实现已提供 `tenant/team` 请求级隔离、知识/配置资源 ACL，以及基于 JWKS 的外部 IAM Bearer JWT + SCIM 目录同步。
- 当前实现已提供轻量知识图谱关系写入与跨仓 repo knowledge map 查询。
