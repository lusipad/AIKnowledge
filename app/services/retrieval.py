...
from app.services.vector_store import (
    ...
    build_knowledge_vector_document,
    create_vector_backend,
)
from app.utils import extract_keywords, keyword_overlap_score, similarity_score
...
def rank_knowledge_items(
    ...
    *,
    ...
    documents = [build_knowledge_vector_document(item) for item in items]
    vector_matches = {
        match.document_id: match for match in vector_backend.score_documents(query, documents, database=database)
    }
...
def rank_config_rules(
    ...
    *,
    ...
    documents = [document for profile in profiles for document in build_config_vector_documents(profile)]
...
def select_config_rules(ranked_rules: list[dict]) -> list[dict]:
    selected: list[dict] = []
    selected_per_scope: dict[str, int] = {'path': 0, 'repo': 0, 'team': 0, 'tenant': 0, 'global': 0}
