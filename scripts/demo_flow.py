...
def run_http_demo() -> None:
    from scripts.http_client import run_demo
    from app.client import build_client_from_env

    result = run_demo(build_client_from_env())
    print('mode: http')
    print('session_id:', result['session_id'])
    print('knowledge_id:', result['knowledge_id'])
    print('request_id:', result['request_id'])
    print('context_summary:', result['context_summary'])


if os.getenv('AICODING_API_BASE_URL'):
    run_http_demo()
else:
    run_local_demo()
